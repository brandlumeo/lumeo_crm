...
export function CustomerTasks({ customerId }: { customerId: number }) {
  const queryClient = useQueryClient();
  const [openDropdown, setOpenDropdown] = useState<number | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ["tasks", "customer", customerId],
    queryFn: () => fetchTaskPage({ customer: customerId, limit: 50 }),
  });

  const updateMutation = useMutation({
    mutationFn: updateTask,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tasks", "customer", customerId] });
