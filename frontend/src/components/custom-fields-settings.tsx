...
export function CustomFieldsSettings() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
                  ...
                  <tr key={cf.id} className="hover:bg-bone-3 transition-colors bg-paper">
                    ...
                    <td className="py-3 px-5 text-right">
                      <button
                        ...
                        disabled={deleteMutation.isPending && fieldToDelete?.id === cf.id}
      ...
      {fieldToDelete && (
        <div className="fixed inset-0 bg-ink/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-bone w-full max-w-sm rounded-xl shadow-2xl border border-line flex flex-col animate-rise">
            <div className="p-6">
              <h3 className="font-serif text-[18px] text-ink mb-2">Delete Custom Field</h3>
              <p className="text-[13px] text-ink-2 leading-relaxed">
                Are you sure you want to delete the custom field <span className="font-semibold text-ink">"{fieldToDelete.label}"</span>? All saved values in leads, customers, or deals will be permanently hidden.
              </p>
            </div>
            <div className="px-6 py-4 border-t border-line bg-bone-2 rounded-b-xl flex justify-end gap-3">
              <button
                type="button"
              ...
              <button
                type="button"
                onClick={() => deleteMutation.mutate(fieldToDelete.id)}
                className="btn px-4 py-2 bg-red-600 hover:bg-red-700 text-white border-transparent flex items-center gap-2 shadow-sm shadow-red-500/20"
                disabled={deleteMutation.isPending}
              >
