...
export function LeadDetailsSlideover({ leadId, open, onOpenChange }: { leadId: number | null, open: boolean, onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      {isLoading ? (
        <div className="p-6 text-muted">Loading lead details...</div>
      ) : error || !lead ? (
        <div className="p-6 text-muted">Lead not found.</div>
      ) : (
        <div className="space-y-6">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-xl font-serif text-ink tracking-tight">{lead.name}</h2>
